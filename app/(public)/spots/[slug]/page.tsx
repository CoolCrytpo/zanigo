...
export default async function SpotFichePage({ params }: PageProps) {
  const { slug } = await params
  let listing = null
  ...
  const jsonLd = {
    ...
    url: `${APP_URL}/spots/${listing.slug}`,
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              {listing.amenities.length > 0 && (
                <div className="card p-5">
                  <h2 className="text-overline mb-3">Sur place</h2>
                  <div className="flex flex-wrap gap-2">
                    {listing.amenities.map((a) => (
                      <span key={a.id} className="text-sm px-3 py-1 rounded-full" style={{ background: 'var(--color-vert-light)', color: 'var(--color-vert)' }}>
                        {a.icon} {a.label}
                      </span>
                    ))}
                  </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              {(listing.address || listing.contact_phone || listing.website_url) && (
                <div className="card p-5">
